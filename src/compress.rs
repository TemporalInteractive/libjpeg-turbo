...
/// Compresses ffi pixel data into JPEG.
#[derive(Debug)]
#[doc(alias = "tjhandle")]
...
impl Compressor {
    ...
    pub fn new() -> Result<Compressor> {
        let mut handle = Handle::new(ffi::TJINIT_TJINIT_COMPRESS)?;
        handle.set(ffi::TJPARAM_TJPARAM_QUALITY, DEFAULT_QUALITY as libc::c_int)?;
        handle.set(
            ffi::TJPARAM_TJPARAM_SUBSAMP,
    ...
    pub fn set_quality(&mut self, quality: i32) -> Result<()> {
        self.handle
            .set(ffi::TJPARAM_TJPARAM_QUALITY, quality as libc::c_int)
    }
    ...
    pub fn set_subsamp(&mut self, subsamp: Subsamp) -> Result<()> {
        self.handle
            .set(ffi::TJPARAM_TJPARAM_SUBSAMP, subsamp as i32 as libc::c_int)
    }
    ...
    pub fn set_optimize(&mut self, optimize: bool) -> Result<()> {
        self.handle
            .set(ffi::TJPARAM_TJPARAM_OPTIMIZE, optimize as libc::c_int)
    }
    ...
    ///
    /// // grab a ffi yuv image from somewhere
...
pub fn compressed_buf_len(width: usize, height: usize, subsamp: Subsamp) -> Result<usize> {
    ...
    let height = height
        .try_into()
        .map_err(|_| Error::IntegerOverflow("height"))?;
    let len = unsafe { ffi::tj3JPEGBufSize(width, height, subsamp as libc::c_int) };
    let len = len
        .try_into()
